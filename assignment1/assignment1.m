...
%
% Answer: The command 'clear all', eliminates everything in the Workspace.
% While the original commands remain in the Command Window, the variables
% are no longer saved.
% 
%
...
% averageAnswer.
% The function used to take the average is "mean".
 vecZ = [1 7 21 32 67 32453]
 mean(vecZ)
 averageAnswer = mean(vecZ)


% 4. What command would you type to see a list of all installed toolboxes?
% The command "ver" shows the version of MATLAB you have, along with a list of
...
%
% Answer: The two major types of loops that MATLAB supports are FOR loops
% and WHILE loops. For loops are the counted loops and are used when
% repeating a function or script is necessary and the number of times the
% satement will be used is known. When the number of times the action will
% be repeated is unknown, a while loop is used. The action is executed as
% long as the condition remains true. It only stops once the condition is
% false.
% 
...
%
% Answer: NaN stands for "Not a Number". For example, 0/0 would result in
% "NaN"
%
...
% - find the maximum.
vecR = randi([-10 10],5,1)
subtractVecR = vecR - 3
posNumVecR = sum(vecR>0)
absoluteVecR = abs(vecR)
maximumVecR = max(vecR)
...
% unitVector. 
xValue = input('Enter a value for x:')
yValue = input('Enter a value for y:')
zValue = input('Enter a value for z:')
u = [xValue yValue zValue]/sqrt((xValue^2) + (yValue^2) + (zValue^2))
unitVector = u
